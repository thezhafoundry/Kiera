...
class SineGen(torch.nn.Module):
    ...
    def forward(self, f0: torch.Tensor, upp: int, sine_noise: torch.Tensor = None):
        ...
        with torch.no_grad():
            ...
            # TRT shim: noise is now passed in as an external ONNX input (shape
            # [1, OUT_PADDED_48K, 1]) generated by numpy's RNG in trt_pipeline.py.
            # This avoids the internal torch.randn_like which emits ONNX RandomNormal
            # that TRT Myelin cannot compile.  Falls back to zeros when None (e.g.
            # during tracing warm-up with dummy inputs — parity check uses a real tensor).
            if sine_noise is not None:
