...
| **Inference Block Accumulation** | up to 160 ms (hop, design target) | `modal_deploy/streaming.py::BlockAccumulator` only pops a block once 160ms of *new* audio has arrived (`BLOCK_MS`/`BLOCK_SAMPLES_IN`); a 560ms left-context window (`CONTEXT_MS`) is prepended for inference quality but doesn't itself add wait time (hop-streaming 2026-07-11 — same 720ms TRT window as before, advanced by a smaller hop). |
...
| **SOLA Crossfade** | 40 ms held back per hop | `modal_deploy/streaming.py::sola_crossfade` (`SOLA_CROSSFADE_SAMPLES`, 1920 samples @ 48kHz) holds back the last 40ms of each converted hop as `pending_tail`, overlap-added onto the next hop, to avoid audible seams between hops (halved from 80ms with the 2026-07-11 hop change to keep the same 25% overlap ratio). |
| **Standing Playout Buffer** | 0.10 s target / 5 s cap (hop-streaming 2026-07-11; history 3s → 1.25s → 0.25s → 0.10s) | `VoiceConversionWorker._PLAYOUT_BUFFER_TARGET_BYTES`/`_MAX_BYTES` in `backend/pipeline.py` — a producer/consumer split that absorbs any hop slower than real-time, trading delay for voice continuity. See CLAUDE.md "Audio Pipeline & Streaming" and `.agents/context/subsystem-notes.md`. At 0.10s it only cushions hop-to-hop jitter; it is no longer the dominant budget term. |
...
| **Steady-state Total** | **~450-550 ms (design estimate incl. network/PSTN; pipeline-added ~370ms — not yet live-measured)** | Dominated by hop accumulation (160ms) and the 0.10s cushion, not GPU inference (measured ~51ms/hop live on 2026-07-11, see the banner). No live spectral-tone measurement has been run against the hop-streaming configuration yet — run §3 after deploying it and replace this estimate. |
