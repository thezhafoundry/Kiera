    ...
    async def _run_conversion_stream(self):
        ...
        try:
            ...
            with contextlib.suppress(asyncio.CancelledError):
                await consumer_task
            # The consumer paces drain to real time (see _run_playout_consumer)
            # and can be mid-wait, holding already-converted audio, at the
            # exact moment the generator above ends -- cancelling it above
            # can leave that audio sitting in _playout_buffer, never
            # published. Flush whatever's left now, immediately/unpaced,
            # rather than silently truncating the tail of the call's audio;
            # a brief blip on this one last fragment is a far smaller cost
            # than losing it outright.
            async with self._playout_buffer_lock:
                leftover = bytes(self._playout_buffer)
                self._playout_buffer.clear()
            if leftover:
                await self._publish_frames(leftover)
                ...
                if chunk:
                    ...
                    if next_publish_time is None or now >= next_publish_time:
                        ...
                        try:
                            ...
                            # chunk is already popped out of _playout_buffer --
                            # it exists only in this local variable now. If we
                            # honor the cancellation here without publishing,
                            # it's gone: not in the buffer for a caller to
                            # flush, not sent to LiveKit either. Publish it
                            # before re-raising so teardown (which cancels
                            # this task mid-pace whenever the converter
                            # stream ends with backlog still queued) can
                            # never silently truncate the tail of the call.
