...
| **TensorRT migration — merged to `main` 2026-07-07 (merge commit `9c1093a`, `trt-migration` branch).** `implementation_plan.md` / `TRT_ROLLOUT_STEPS.md` (repo root) are the spec; another model implemented, this agent reviewed across 3 rounds. All round-3 hardening findings landed as real commits (fail-closed engine check, FAISS fail-fast, provider asserts, `modal_defs.py` single-source image, hubert export fix, vendored ONNX-export shims in `models_onnx.py`/`attentions_onnx.py` now **committed**, not just working-tree). C3 GPU benchmark passed on live L4 (ap-southeast): median 66ms / p95 68ms vs. ≤400ms gate — see [[log]]. `USE_TRT=1` is wired into the `fastapi_app` env in `modal_deploy/worker.py`, but whether `modal deploy` has actually been run to push this live is unverified — confirm via `/api/health` (`"engine": "trt"`) before assuming it's serving TRT in production, don't infer from committed code alone (see [[feedback_infra-drift]]). Remaining: C4 (offline A/B WAVs) and C5 (listen test for the deterministic-noise shim's breathiness tradeoff) from plan Task 9, then Task 10 rollout confirmation (USER-RUN). Keep-warm stays env-gated OFF by default (`RVC_KEEPWARM=1` to enable). | High | **Merged to main 2026-07-07** — C4/C5 + live-rollout confirmation pending (see [[log]], [[subsystem-notes]]) |
