...
| **Control plane is unauthenticated**: the `KEIRA_CONTROL_TOKEN` bearer-auth gate on operator routes was deliberately removed 2026-07-19 (see [[log]]) — every operator/dashboard route and the call-events WS are now open to anyone who can reach the URL. Twilio signature validation and `CORS_ORIGINS` restriction are unaffected. Re-add auth before any public deployment if this wasn't intentional long-term. | P0 | Regressed by request 2026-07-19; not committed/pushed |
